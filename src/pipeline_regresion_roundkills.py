...
# %% [markdown]
# # Pipeline CS:GO – Regresión RoundKills (ventana 0-30 s)
...
TARGET       = "RoundKills"
TIME_WINDOW  = 30
RAW_PATH     = "/mnt/data/Anexo_ET_demo_round_traces_2022.csv"
...
df["TimeAlive_norm"] = (df["TimeAlive"]-df["TimeAlive"].min())/(df["TimeAlive"].max()-df["TimeAlive"].min())
df["TravelledDistance_norm"] = (df["TravelledDistance"]-df["TravelledDistance"].min())/(df["TravelledDistance"].max()-df["TravelledDistance"].min())
df["IsPistolRound"] = df["RoundStartingEquipmentValue"] < 1500
df["HeadshotRatio"] = np.where(df[TARGET]>0, df["RoundHeadshots"] / df[TARGET], 0)
df["EconomicEfficiency"] = df[TARGET] / (df["RoundStartingEquipmentValue"] + 1)
if {"RLethalGrenadesThrown","RNonLethalGrenadesThrown"}.issubset(df.columns):
    df["TotalGrenades"] = df["RLethalGrenadesThrown"] + df["RNonLethalGrenadesThrown"]

poly = ["RoundAssists","RoundHeadshots","HeadshotRatio","EconomicEfficiency"]
for c in poly:
    df[f"{c}_sq"] = df[c]**2
...
df["EquipCategory"] = pd.qcut(df["RoundStartingEquipmentValue"],4,labels=False)

for c in df.select_dtypes(include=["int64","float64"]).columns:
    df[c] = mstats.winsorize(df[c], limits=(0.05,0.05))
...
models = {
    ...
    "RandForest": RandomForestRegressor(n_estimators=400, random_state=RANDOM_STATE, n_jobs=-1),
    "GradBoost": GradientBoostingRegressor(n_estimators=600, learning_rate=0.05, random_state=RANDOM_STATE),
}
if CATBOOST_AVAILABLE:
    models["CatBoost"] = CatBoostRegressor(depth=6, learning_rate=0.05, iterations=700,
...
MODEL_PATH = f"models/{best_name}_roundkills.pkl"
